(async function() { 
    ...
    let testingOnlyWarning = `<br><div><span style="color:red; font-weight:bold;">
    IMPORTANT:</span> This API server is <u>only for testing</u>. It is purposely 
    handicapped to only output up to 10 results per generator, per day (except 
        for the animal-sentence generator). Please follow the instructions <a 
        href="https://perchance.org/diy-perchance-api" target="_blank">here</a> 
        to set up your own server (it's free!), and you'll be able to generate as 
        many results from as many generators as you like :)</div>`;
    app.get("/", (request, response) => {
      response.send(`
        <div style="font-family: monospace; line-height: 1.4rem;">
          <div>Use <span style="background: #e5e5e5;padding: 0.25rem;"><a 
          style="text-decoration:none; color:inherit;" href=
          "https://${process.env.PROJECT_DOMAIN}.glitch.me/api?generator=animal-
          sentence&list=output">https://${process.env.PROJECT_DOMAIN}.glitch.me/api?
          generator=<span style="background:#ffd04a;">animal-sentence</span>&amp;
          list=<span style="background:#ffd04a;">output</span></a></span> to generate
          some text. Loading a new generator for the first time will take several 
          seconds, but after that it will be cached and you should be able to generate
           results quickly.</div>
    ...
    async function makeGeneratorWindow(generatorName) {
      let html = await fetch(
        `https://perchance.org/api/downloadGenerator?generatorName=${generatorName}&__
        cacheBust=${Math.random()}`).then(r => r.text());
